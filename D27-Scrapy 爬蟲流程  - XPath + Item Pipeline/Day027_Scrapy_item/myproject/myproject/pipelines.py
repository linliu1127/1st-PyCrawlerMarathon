...
# See: https://scrapy-chs.readthedocs.io/zh_CN/0.24/topics/item-pipeline.html
import os
import json
import scrapy
...
class JSONPipeline(object):
    def open_spider(self, spider):
        self.start_crawl_datetime = datetime.now().strftime('%Y%m%dT%H:%M:%S')#ex:20200205T00:11:52

        # 在開始爬蟲的時候建立暫時的 JSON 檔案
        # 避免有多筆爬蟲結果的時候，途中發生錯誤導致程式停止會遺失所有檔案
        self.dir_path = Path(__file__).resolve().parents[1] / 'crawled_data'#https://juejin.im/post/5bf790a4518825689f1b2b88 parents後面數字代表回到第幾個父目錄
        self.runtime_file_path = str(self.dir_path / '.tmp.json.swp')#/的功能是在路徑尾端添加東西(此為WindowsPath這種type的運算)
    ...
    def process_item(self, item, spider):
        # 把資料轉成字典格式並寫入文件中
        if not isinstance(item, dict):#https://www.runoob.com/python/python-func-isinstance.html
            item = dict(item)
    ...
    def close_spider(self, spider):
        ...
        self.store_file_path = self.dir_path / '{}-{}.json'.format(self.start_crawl_datetime,
                                                                   self.end_crawl_datetime)#type(self.store_file_path)=WindowsPath
